...
class PPO():
    ...
    def establish_pytorch_graph(self, flag, sample, n):
        ...
        # motivation indices regression
        mi_loss = 0
        # part1
        SAFE_LIMIT = 11
        filter = (real_threat<SAFE_LIMIT) & (real_threat>=0)
        assert filter.shape[-1]==1
        assert real_threat.shape[-1]==1
        filter = filter.squeeze()
        threat_loss = F.mse_loss(
            others['mi_threat'][filter], 
            repeat_at(real_threat.squeeze(-1)[filter], insert_dim=-1, n_times=others['mi_threat'].shape[-1])
        )
        mi_loss += threat_loss
        # part2
        assert real_value.shape[-1]==1
        mi_vloss = F.mse_loss(
            others['mi_value'], 
            repeat_at(real_value.squeeze(-1), insert_dim=-1, n_times=others['mi_value'].shape[-1])
        )
        mi_loss += mi_vloss
        ...
        CT_net_loss = value_loss * 1.0 + mi_loss * 0.1 
