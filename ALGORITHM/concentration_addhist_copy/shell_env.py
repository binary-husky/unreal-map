...
class ShellEnvWrapper(object):
    ...
    def interact_with_env(self, State_Recall):
        ...
        obs_feed_in, his_pool_next = self.solve_duplicate_old_version(obs_feed, his_pool_obs_feed)
    ...
    def solve_duplicate_old_version(self, obs_feed_new_arg, prev_his_pool_arg):
        #  input might be (n_thread, n_agent, n_entity, basic_dim), or (n_thread, n_agent, n_entity*basic_dim)
        # both can be converted to (n_thread, n_agent, n_entity, basic_dim)
        obs_feed_new = my_view(obs_feed_new_arg, [0, 0, -1, self.n_basic_dim])
        prev_obs_feed = my_view(prev_his_pool_arg, [0, 0, -1, self.n_basic_dim])
        ...
        next_his_pool = roll_hisory(obs_feed_new, prev_obs_feed, valid_mask, N_valid, np.zeros_like(prev_obs_feed))

        return obs_feed, next_his_pool


    def solve_duplicate(self, obs_feed_new_arg, prev_his_pool_arg):
        #  input might be (n_thread, n_agent, n_entity, basic_dim), or (n_thread, n_agent, n_entity*basic_dim)
        # both can be converted to (n_thread, n_agent, n_entity, basic_dim)
        obs_feed_new = my_view(obs_feed_new_arg, [0, 0, -1, self.n_basic_dim])
        prev_obs_feed = my_view(prev_his_pool_arg, [0, 0, -1, self.n_basic_dim])
