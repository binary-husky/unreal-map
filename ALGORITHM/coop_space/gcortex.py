...
from my_utils import my_view, Args2tensor_Return2numpy, Args2tensor
...
class Action_Extraction_Module(nn.Module):
    def __init__(self, hidden_dim=128):
        super().__init__()
        self.reverse_container = GlobalConfig.reverse_container
        activation_func = nn.ReLU
        h_dim = hidden_dim
        self.attn_right = MultiHeadAttention(n_heads=1, input_dim=h_dim, embed_dim=h_dim)
...
class GNet(nn.Module):
    def __init__(self, num_agents, num_entities, K_EMod=2 ,hidden_dim=128):
        super().__init__()
        n_cluster = GlobalConfig.g_num
        n_agent = num_agents
        self.n_agent = n_agent
        h_dim = hidden_dim
        n_entity = num_entities
        basic_vec_len = GlobalConfig.basic_vec_len
        activation_func = nn.ReLU

        if not GlobalConfig.one_more_container:
        ...
        self.test_mode = GlobalConfig.eval_mode
    ...
    def _act(self, all_emb, eval_mode=False, eval_actions=None):
        eval_top_act, eval_bottom_act = (eval_actions[:, :2], eval_actions[:,2:(self._n_cluster+2)]) if eval_mode \
                                   else (None,                    None                 )

        top_feature, logits_cluster_entity, value_top, value_bottom, others = self.get_feature(all_emb, eval_mode=eval_mode)

        act_top, actLogProbs_top, distEntropy_top, probs_top = self.get_fifo_act(net=self.fifo_net_top, feature=top_feature, eval_mode=eval_mode, eval_actions=eval_top_act)

        act_bottom, actLogProbs_bottom, distEntropy_bottom, probs_bottom = self.get_fifo_act_new(logits_cluster_entity, eval_mode=eval_mode, eval_actions=eval_bottom_act)
        ...
        if not eval_mode:
            ...
            return value_top, actLogProbs_top, distEntropy_top, probs_top, \
                value_bottom, actLogProbs_bottom, distEntropy_bottom, probs_bottom, others
