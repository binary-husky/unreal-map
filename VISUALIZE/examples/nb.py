    ...
    for step in range(100000):
        ...
        if step%10==0:
            for body in range(N体):
                ...
                可视化桥.发送几何体(
                    'ball|%d|%s|%.2f'%(body, colors[body], size),  # 填入核心参量： “已声明的形状|几何体之ID标识|颜色|整体大小”
                    位置[body, 0], 位置[body, 1], 位置[body, 2], ro_x=0, ro_y=0, ro_z=0, # 6 DOF
                    opacity=1, label='',  track_n_frame=20,  track_tension=0, track_color='white',    # 轨迹的颜色显示，输入js颜色名或者hex值均可
                    )
        可视化桥.结束关键帧()
