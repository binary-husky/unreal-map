...
class DebugAgentRed(Agent):
    def __init__(self, name, config):
        super(DebugAgentRed, self).__init__(name, config["side"])
        self.my_uvas_infos = []
        self.my_manned_info = []
        ...
        import json
        with open('debug_conf.json','r') as f:
            self.conf = json.load(f)
            print(self.conf)

        self.attack_dis = self.conf['attack_dis']

    def get_one_alive_uav(self):
        for id_ in self.my_planes:
            if self.my_planes[id_].is_drone and self.my_planes[id_].alive:
                return { id_:self.my_planes[id_] }
        return {}
    # get class object
    def get_alive_uav(self):
        return { id_:self.my_planes[id_] for id_ in self.my_planes if self.my_planes[id_].is_drone and self.my_planes[id_].alive}
    ...
    def Full_Assult(self):
        ...
        my_vip = self.get_vip()
        for id_ in self.my_planes:
            p = self.my_planes[id_]
            p.step_init()

        my_uav = self.get_one_alive_uav()
        for pid in my_uav:
            if not my_uav[pid].alive:
                return
        ...
        if 'ID_Bind' not in self.state_recall:
            ...
            n_op = len(op_dict)
            for i,id_ in enumerate(my_uav):
                p = my_uav[id_]
                if not p.alive: continue
                p.bind = op_dict[i]['ID'] if i < n_op else op_dict[n_op-1]['ID']
                self.state_recall['ID_Bind'] = ''

        # deal ms
        for ms in self.enemy_missile_infos:
            my_plane_targeted = ms['EngageTargetID']
            if my_plane_targeted not in self.my_planes: continue
        ...
        for pid in my_uav:
            ...
            if len(p.under_attack) >= 1:
                dis = [get_dis(pid, ms['ID']) for ms in p.under_attack]
                min_ms_dis = min(dis)
                if min_ms_dis <= 55e3:
                    p.state = 'escape'
        ...
        def attack_cmd(uav):
            bind_id = uav.bind
            if bind_id in self.active_index:
                dis = get_dis(uav.ID, bind_id)
