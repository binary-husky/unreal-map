...
import numpy as np
import sys
sys.path.append('./MISSIONS/gym_fortattack_ma')
...
def make_fortattack_env(env_id, rank):
    # scenario = gym.make('fortattack-v1')
    scenario = FortAttackEnvV1( numguards=ScenarioConfig.num_guards, 
    ...
    if ScenarioConfig.benchmark:
        env = FortAttackGlobalEnv(world, scenario.reset_world, scenario.reward, scenario.observation, scenario.benchmark_data, id=rank)
    else:
        env = FortAttackGlobalEnv(world, scenario.reset_world, scenario.reward, scenario.observation, id=rank)
    return env
